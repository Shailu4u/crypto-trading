...
export default function Notification({}: Props) {
  const { notifications, setNotification } = useStore((state) => ({
    notifications: state.notifications,
    setNotification: state.setNotification,
  }));
  const [notifyCls, setNotifyCls] = useState("");
  ...
  return (
    <>
      {notifications.map((note) => (
        <div
          key={note.type}
          className="fixed bottom-12 right-5 flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800"
          role="alert"
        >
          <div className={notifyCls}>
            {note.type === "error" && (
              <svg
                className="w-5 h-5"
                fill="currentColor"
